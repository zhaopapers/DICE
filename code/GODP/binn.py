...
class BINN(pl.LightningModule):
    ...
    def on_test_epoch_end(self):
        ...
        positive_class_probabilities = test_probs[:, 1]
        ...
        AUC = self.calculate_AUC(test_labels, positive_class_probabilities)
        ...
        return { "test_acc": accuracy, "test_AUC": AUC, "test_F1": F1}
...
def _generate_residual(
    layer_sizes, connectivity_matrices=None, activation="tanh", activation_final= "sigmoid", bias=False, n_outputs=2
):
    layers = []
    ...
    for res_index in range(len(layer_sizes)):
        if res_index == len(layer_sizes) - 1:
            ...
            if activation_final == "tanh":
                layers.append((f"Residual_Tanh_final", nn.Tanh()))
            elif activation_final == "relu":
                layers.append((f"Residual_ReLU_final", nn.ReLU()))
            elif activation_final == "leaky relu":
                layers.append((f"Residual_LeakyReLU_final", nn.LeakyReLU()))
            elif activation_final == "sigmoid":
                layers.append((f"Residual_Sigmoid_final", nn.Sigmoid()))
            elif activation_final == "elu":
                layers.append((f"Residual_Elu_final", nn.ELU()))
            elif activation_final == "hardsigmoid":
                layers.append((f"Residual_HardSigmoid_final", nn.Hardsigmoid()))            
        else:
            layers = generate_block(res_index, layers, activation)
