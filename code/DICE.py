...
class based_cell_train:
    def __init__(self, model: BINN, temperature_init=1.0):
        self.model = model
    ...
    def fit(
        ...
        for iteration in range(nr_iterations):
            ...
            self.test_data = torch.Tensor(np.concatenate([X_train, X_val, X_test], axis=0)).to(self.device)
            self.y = torch.Tensor(np.concatenate([y_train, y_val, y_test], axis=0)).to(self.device)
